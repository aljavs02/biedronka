...
super %>%
  ...
  ggplot(aes(x = m, y = gross_income)) + geom_bar(stat = "identity", fill = "steelblue") + labs(title = "Dochodowość według pory dnia", x = "Pora dnia", y = "Dochód") + theme_bw()
```
...
super %>% 
  ...
  summarize(Min=min(gross.income),
            ...
            Skośność=skew(gross.income),
            Kurtoza=kurtosi(gross.income)) %>%
kbl(
    caption = "Tabela 1. Statystyki rozkładu gross.income wg branchu.",
    digits = 2,
    align = "lccccccccc",
    col.names = c("Branch", "Min", "Max", "Średnia", "Odchylenie std.", "Mediana", "Q1", "Q3", "Skośność", "Kurtoza")
  ) %>%
  kable_styling(
  ...
  row_spec(0, bold = TRUE, background = "#e57373", color = "#222222") %>%  
  row_spec(2, bold = F, background = "#ffebee", color = "#222222") %>%    
  row_spec(c(1, 3), bold = F, background = "#ffcdd2") %>%  
  footnote(
    general = "Tabela przedstawia podstawowe statystyki opisowe zmiennej gross.income w podziale na oddziały.",
    footnote_as_chunk = TRUE
  )
```

Powyższy kod genruje wyniki minimalnych, maksymalnych i średnich
wartości zmiennych przyjmujących typ numeryczny i liczb całkowitych oraz
określa wartości mediany, 1. oraz 3. kwartla.

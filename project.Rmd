...
ggplot(super, aes(x = Rating, fill = factor(Payment))) +
  geom_density(alpha = 0.6, color = NA) +  
  scale_fill_manual(values = c("#2b8cbe", "#bae4bc", "#b2e2e2"), labels = c("Gotówka", "Karta kredytowa", "Portfel elektroniczny")) + 
  labs(
...
- Mediany są bliskie średnim, co sugeruje symetryczny rozkład cen. N
- iskie wartości skośności i kurtozy potwierdzają brak znaczących odchyleń od normalnego rozkładu cen w analizowanych liniach produktów.
...
anova_result1 <- aov(gross.income ~ Branch, data = super)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
super$Payment <- as.factor(super$Payment)
anova_result1 <- aov(gross.income ~ Payment, data = super)

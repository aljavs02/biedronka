...
Celem tego części jest kompleksowe oczyszczenie i przygotowanie danych
...
odpowiednich technik eksploracji, analizy oraz imputacji danych, walidacji końcowy
...
Wyznaczono następujące braki w kolumnach: Rating (150), City
(100), gross.income (150). Sprawdzono także 
-procent braków wstosunku do całego zbioru danych 
-podsumowano braki dla poszczególnych kolumn 
...
Została wyznaczona tabela częstości dla kolumn Gender, zostały wyświetlone unikalne
wartości w kolumnie rating oraz został zbudowany wykres pudełkowy dla
kolumny Rating z zaznaczeniem wartości odstających. Dokonano także konwersji kolumny Gender na zmienną kategoryczną jako Sex oraz wyznaczona tabela częstości dla nowej kolumny Sex.
...
```
W tym kroku została dokonana imputacja braków metodą KNN. Dla
...
```
W procesie walidacji dostrzeżono nieścisłości dla ponad 300 rekordów w odniesnieu do wartości Total. Zatem nadpisano tę kolumnę prawidłowym wzorem na jej zawartość na podstawie wartości z  pozostałych kolumn.
Proces oczyszczania oraz walidacji danych został zakończony. Zastosowane metody

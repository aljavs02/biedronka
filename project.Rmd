...
sum <-
  list("Ratio" =
       list("Min"= ~ min(gross.income),
            "Max"= ~ max(gross.income),
            "Kwartyl dolny"= ~ quantile(gross.income,0.25),
            "Mediana"= ~ round(median(gross.income),2),
            "Kwartyl górny"= ~ quantile(gross.income,0.75),
            "Średnia"= ~ round(mean(gross.income),2),
            "Odch. std."= ~ round(sd(gross.income),2),
            "IQR"= ~ round(iqr(gross.income),2),
            "Odchylenie ćwiartkowe"=~round(iqr(gross.income)/2,2),
            "Odch. std. w %"=~round((sd(gross.income)/mean(gross.income)),2),
            "Odch. ćwiartkowe w %"=~round((iqr(gross.income)/median(gross.income)),2),
            "Skośność"=~round(skew(gross.income),2),
            "Kurtoza"=~round(kurtosi(gross.income),2)
            ))
tabela <- summary_table(super, summaries = sum, by = "Branch")
view(tabela)
knitr::kable(tabela,
  digits = 2,
  align = "lcccc",
  caption="Tabela 1. gross.income wdg branchu.",
  col.names = c("stat", "A", "B", "C")) 
```
nie działa mi to - to be investigated, sprawdźcie jak u Was
...
super %>%
  ...
  tbl_summary(
    ...
    statistic = all_continuous() ~ c(
      ...
      "{min}, {max}"),
    missing = "no",
    label = gross.income ~ "gross.income") %>%
  modify_header(label ~ "**Zmienna**") %>%
  modify_caption("**Tabela 1. Rozkład gross.icome wdg filii**") %>%
  bold_labels() %>% 
  add_p(pvalue_fun = ~ style_pvalue(.x, digits = 2))
```
czemu to się to Date dodaje - ja pierdziele

Powyższy kod genruje wyniki minimalnych, maksymalnych i średnich wartości zmiennych przyjmujących typ numeryczny

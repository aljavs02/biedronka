...
(1000) i ilość zmiennych (17). Analiza pozwala zauważyć, iż zmienna Date zawierająca datę zakupu nie jest wyrażona w odpowiednim formacie. Przy użyciu pakietu *lubridate* nastąpi zmiana formatu z character na date.
...
```{r}
# 1. Obliczanie skumulowanego przychodu
super <- super %>% 
  group_by(Date, Branch) %>% 
  mutate(Daily_Total_Revenue = sum(Total, na.rm = TRUE))

# 2. Dodanie kolumny z miesiącem
super <- super %>%
  mutate(Month = month(Date, label = TRUE, abbr = FALSE))


months <- sort(unique(super$Month))
plots <- list()
...
for (Month in months) {
  ...
  plot <- ggplot(monthly_data, aes(x = Date, y = Daily_Total_Revenue, color = Branch, group = Branch)) +
    geom_line(size = 1.2) +
    labs(
      title = paste("Dzienne zarobki dla miesiąca:", month),
      x = "Data",
      y = "Dzienne zarobki ($)",
      color = "Oddział"
    ) +
...
```{r}
# Obliczanie skumulowanego przychodu
cumulative_revenue <- super %>%
  group_by(Branch, Date) %>%
  summarise(Daily_Total_Revenue = sum(Total, na.rm = TRUE)) %>%
  arrange(Branch, Date) %>%
  mutate(Cumulative_Revenue = cumsum(Daily_Total_Revenue))

# Dodanie podziału na tygodnie i miesiące
cumulative_revenue <- cumulative_revenue %>%
  mutate(
    Week = format(as.Date(Date), "%Y-%U"),
    Month = format(as.Date(Date), "%Y-%m")
  )
View(cumulative_revenue)
```

```{r}
weekly_revenue <- cumulative_revenue %>%
  group_by(Branch, Week) %>%
  summarise(Weekly_Cumulative_Revenue = max(Cumulative_Revenue))

ggplot(weekly_revenue, aes(x = Week, y = Weekly_Cumulative_Revenue, color = Branch, group = Branch)) +
  geom_line(size = 1.2) +
  labs(
    title = "Skumulowany przychód dla każdego oddziału (podział tygodniowy)",
    x = "Tydzień",
    y = "Skumulowany przychód ($)",
    color = "Oddział"
  ) +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

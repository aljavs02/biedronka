...
vis_miss(super)
gg_miss_fct(super, fct = Gender)  # rozdzielic na oddzielny chunk
gg_miss_upset(super) 
```
...
super %>%
  ...
  ggplot(aes(x = m, y = gross_income)) + geom_bar(stat = "identity", fill = "steelblue") + labs(title = "Dochodowość według pory dnia", x = "Pora dnia", y = "Dochód") + theme_minimal()
```
Na podstawie przedstawionego wykresu można ustalić, że najbardziej dochodwy okres dla supermarketu występuje między godzinami 19 a 20. Przedstawiony wykres użyteczny może okazać się przy ustalaniu i modyfikacji godzin otwarcia sklepu np. w dni świąteczne, bądź weekendy.
Najmniej dochodowy moment dla supermarketu znajduje się w ostatniej godzinie otwarcia sklepu.
...
ggboxplot(super, x = "Branch", y = "Rating", 
          ...
          legend = "none") + scale_y_continuous(breaks = seq(1, 10, 0.5)) + theme_minimal()
```
